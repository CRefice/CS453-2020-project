...
private:
  ...
  public:
    ...
    Shared<Balance[]> accounts; // Amount of money on the accounts (undefined if
                                // not allocated)
  public:
    /** Deleted copy constructor/assignment.
     **/
    ...
    AccountSegment(Transaction& tx, void* address)
        : count{tx, address}, next{tx, count.after()}, parity{tx, next.after()},
          accounts{tx, parity.after()} {}
  };
...
private:
  ...
  float
      prob_long; // Probability of running a long, read-only control transaction
...
public:
   ...
   * @param library       Transactional library to use
   * @param nbworkers     Total number of concurrent threads (for both 'run' and
   *'check')
   * @param nbtxperwrk    Number of transactions per worker
   * @param nbaccounts    Initial number of accounts and number of accounts per
   *segment
   * @param expnbaccounts Initial number of accounts and number of accounts per
   *segment
  ...
  WorkloadBank(TransactionalLibrary const& library, size_t nbworkers,
      ...
      : Workload{library, AccountSegment::align(),
        ...
        prob_long{prob_long}, prob_alloc{prob_alloc}, barrier{nbworkers} {}
...
private:
          ...
          while (true) {
            ...
            if (!segment_next) { // Currently at the last segment
              if (count > trigger && likely(count > 2)) { // Deallocate
                --segment_count;
                auto new_parity = segment.parity.read() +
                ...
                if (segment_count < nbaccounts) { // Just "allocate" account
                  ...
                  AccountSegment next_segment{
                      tx, segment.next.alloc(AccountSegment::size(nbaccounts))};
  ...
  /** Short read-write transaction, transferring one unit from an account to an
   *account (potentially the same).
...
public:
  virtual char const* init() const {
    ...
    auto correct =
        transactional(tm, Transaction::Mode::read_only, [&](Transaction& tx) {
          AccountSegment segment{tx, tm.get_start()};
  ...
  virtual char const* check(Uid uid, Seed seed [[gnu::unused]]) const {
    ...
    for (size_t i = 0; i < nbtxperwrk; ++i) {
      ...
      auto correct = transactional(tm, Transaction::Mode::read_write,
                                   [&](Transaction& tx) {
                                     Shared<size_t> counter{tx, tm.get_start()};
