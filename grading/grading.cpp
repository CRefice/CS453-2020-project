...
public:
  ...
  Sync(unsigned int nbworkers)
      : nbworkers{nbworkers}, nbready{0}, status{Status::Done}, errmsg{
                                                                    nullptr} {}
...
public:
   ...
   * @return Total execution time on success, or error constant null-terminated
   *string on failure
   **/
  ...
  master_wait(Chrono::Tick maxtick = Chrono::invalid_tick) {
    ...
    if (!donelatch.wait(maxtick))
      throw Exception::BoundedOverrun{
          "Transactional library takes too long to process the transactions"};
  ...
  bool worker_wait() noexcept {
    ...
    if (res + 1 == nbworkers) { // Latest worker, switch to run status
      ...
      status.store(
          Status::Run,
          ::std::memory_order_release); // Synchronize-with previous worker
                                        // waiting for run/abort state
    } else
      do { // Not latest worker, wait for run status
        short_pause();
        auto res = status.load(
            ::std::memory_order_acquire); // Synchronize-with latest worker
                                          // switching to run/abort state
  ...
  void worker_notify(char const* error) noexcept {
    ...
    auto&& res = nbready.fetch_add(
        1, ::std::memory_order_acq_rel); // Synchronize-with previous worker(s)
                                         // potentially setting aborted status
    if (res + 1 == nbworkers) { // Latest worker, switch to done/fail status
      nbready.store(0, ::std::memory_order_relaxed);
      status.store(status.load(::std::memory_order_relaxed) == Status::Abort
                       ? Status::Fail
                       : Status::Done,
...
static auto measure(Workload& workload, unsigned int const nbthreads,
  ...
  ::std::mutex cerrlock; // To avoid interleaving writes to 'cerr' in case more
                         // than one thread throw
  Sync sync{nbthreads};  // "As-synchronized-as-possible" starts so that threads
  ...
  for (unsigned int i = 0; i < nbthreads; ++i) { // Start threads
    try {
          ...
          [&](unsigned int i) {
            try {
              ...
              sync.worker_notify(workload.check(
                  i, std::random_device{}())); // Random seed is wanted here
              // Synchronized quit
              if (!sync.worker_wait())
                return;
              throw Exception::Unreachable{
      ...
      for (unsigned int j = 0; j < i;
           ++j) // Detach threads to avoid termination due to attached thread
                // going out of scope
        threads[j].detach();
      throw;
    }
  ...
  try {
    ...
    { // Performance measurements (with cheap correctness tests)
      ...
      ::std::nth_element(times, times + posmedian,
                         times +
                             nbrepeats); // Partition times around the median
    }
    { // Correctness check
      sync.master_notify();
...
int main(int argc, char** argv) {
  try {
    ...
    if (argc < 3) {
      ::std::cout << "Usage: " << (argc > 0 ? argv[0] : "grading")
    ...
    for (auto i = 2; i < argc; ++i) {
      ::std::cout << "⎧ Evaluating '" << argv[i] << "'"
                  << (maxtick_init == Chrono::invalid_tick ? " (reference)"
                                                           : "")
                  << "..." << ::std::endl;
      // Load TM library
      TransactionalLibrary tl{argv[i]};
      // Initialize workload (shared memory lifetime bound to workload: created
      ...
      try {
        ...
        auto res = measure(bank, nbworkers, nbrepeats, seed, maxtick_init,
                           maxtick_perf, maxtick_chck);
        ...
        ::std::cout << "⎪ Total user execution time: " << (perfdbl / 1000000.)
                    << " ms";
        ...
        ::std::cout << ::std::endl;
        ::std::cout << "⎩ Average TX execution time: " << (perfdbl / pertxdiv)
                    << " ns" << ::std::endl;
